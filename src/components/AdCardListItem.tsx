...
export default function AdCardListItem(ad: Ad) {
  ...
  return (
    <Link href={`/listings/${id}`} className="group">
      <Card className="overflow-hidden flex flex-col sm:flex-row transition-all duration-300 hover:shadow-lg hover:bg-muted/50">
        ...
        <CardContent className="p-4 flex flex-col justify-between w-full">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mt-auto">
                ...
                <p className="text-xl font-bold text-primary whitespace-nowrap text-left sm:text-right">
                    {formatPrice(price)}
                </p>
            </div>
        </CardContent>
